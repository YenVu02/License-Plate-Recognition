...
# Danh sách các tỉnh thành (Lưu ý: Danh sách này có thể không bao gồm tất cả các tỉnh thành)
provinces = [
    "zh_cuan", "川",
    "zh_e", "鄂",
    "zh_gan", "赣",
    ...
    def train_svm(self):
        ...
        if os.path.exists("svmchinese.dat"):
            ...
            for root, dirs, files in os.walk("train\\charsChinese"):
                ...
                index = provinces.index(pinyin) + PROVINCE_START + 1  # 1 là ký tự Trung Quốc tương ứng với pinyin
                for filename in files:
                    filepath = os.path.join(root, filename)
                    digit_img = cv2.imread(filepath)
                    digit_img = cv2.cvtColor(digit_img, cv2.COLOR_BGR2GRAY)
